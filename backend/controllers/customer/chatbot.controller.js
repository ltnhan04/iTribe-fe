    ...
    try {
      ...
      if (aiAnalysis.intent === "order_tracking") {
        if (aiAnalysis.subIntent === "list_orders") {
          ...
          const orderInfo = await ChatbotService.trackOrder(
            aiAnalysis.orderId,
            req.user._id
          );
          context.orderInfo = orderInfo;
          response =
            `Thông tin đơn hàng ${aiAnalysis.orderId}:\n` +
          ...
          response =
            "Vui lòng cung cấp mã đơn hàng để kiểm tra hoặc nhập 'đơn hàng của tôi' để xem danh sách đơn hàng.";
        }
      } else if (aiAnalysis.intent === "product_search") {
        const searchQuery =
          aiAnalysis.searchQuery ||
          message
            .replace(
              /sản phẩm|hàng|giá|tìm|kiếm|mua|bán|giá bao nhiêu|bao nhiêu tiền|còn|không|nào/gi,
              ""
            )
            .trim();
        ...
        if (!province) {
          ...
          const shippingInfo = await ChatbotService.calculateShippingFee(
            province
          );
          context.shippingInfo = shippingInfo;
          response =
            `Phí ship đến ${province} (${shippingInfo.region}):\n` +
        ...
        const vouchers = await ChatbotService.getAvailableVouchers(
          req.user._id
        );
