...
const IPad = () => {
  ...
  const ipadCategory = APPLE_CATEGORIES.find(
    (category) => category.name.toLowerCase() === "ipad"
  );
  const ipadId = ipadCategory?.id;

  const { data: subCategoriesData, isLoading: isLoadingSubCategories } =
    useSubCategory(ipadId as string);
  const subCategories = subCategoriesData?.data || [];

  const { data: products, isLoading: isLoadingProducts, error: productsError } =
    useGetProductByCategory(selectedSubCategory || (ipadId as string));
