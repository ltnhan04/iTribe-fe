...
const MacBook = () => {
  ...
  const macbookCategory = APPLE_CATEGORIES.find(
    (category) => category.name.toLowerCase() === "macbook"
  );
  const macbookId = macbookCategory?.id;

  const { data: subCategoriesData, isLoading: isLoadingSubCategories } =
    useSubCategory(macbookId as string);
  const subCategories = subCategoriesData?.data || [];

  const { data: products, isLoading: isLoadingProducts, error: productsError } =
    useGetProductByCategory(selectedSubCategory || (macbookId as string));
