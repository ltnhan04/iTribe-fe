...
const IPhone = () => {
  ...
  const iphoneCategory = APPLE_CATEGORIES.find(
    (category) => category.name.toLowerCase() === "iphone"
  );
  const iphoneId = iphoneCategory?.id;

  const { data: subCategoriesData, isLoading: isLoadingSubCategories } =
    useSubCategory(iphoneId as string);
  ...
  const {
    ...
    error: productsError,
  } = useGetProductByCategory(selectedSubCategory || (iphoneId as string));
